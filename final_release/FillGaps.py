...
def fill_by_mean(classes,data,mask):
    ...
    for cl in classes:
        ...
        for i in range(m):
            d_col=dt_cl[:,i]
            missed = np_where(msk[:,i])
            not_missed_in_class=np_where(np_logical_and(~ msk[:,i],msk_cl))
            data[missed,i]=np_mean(data[not_missed_in_class,i])
    return data
...
def get_new_partition(n,r,data):
    ...
    new_ind_dist = [0]
    while len(ind)>1:
        min_dist = np_iinfo(np_int16).max
        min_dist_ind = -1
        ...
        x0 = np_mean(data[new_ind],axis=0)
    ...
    d_dist = np_diff(new_ind_dist)
    # точки, где максимальные "скачки" расстояния
    max_dist_points = np_sort(heapq.nlargest(r, range(len(d_dist)), d_dist.take))
