...
################################################################################
    ...
    def load_datafile( self, fin ) :
        ...
        self.__dataset = np_genfromtxt( fin.name, delimiter = ',' )
...
## Write original data (from datafile)
        ...
        for i in range(n_rows):
            for j in range(n_columns):
                if np_isnan(self.__original[i][j]):
                    sheet0.write(i + 1, j, '')
                else:
        ...
        for i in range(len(self.__current_partition)):
            ...
            centroid = np_mean( cluster, axis = 0 )
            sdev = np_std( cluster-centroid, axis = 0 )
            for j in range( n_columns ) :
...
## Fill missing values if necessary
        ...
        missing_mask = np_any( np_isnan( data ), axis = 1 )
        if not original :
            if np_any( missing_mask ) :
                data = fg.fill_missing( data )
## Prepare data for plottting
        if data.shape[ 1 ] > 3 :
            if original and np_any( missing_mask ) :
## Take the first three dimensions
                d3d = data[:,:3]
            else :
## Do the projection onto the 
                mu = np_mean( data, axis = 0 )
                sigma = np_std( data, axis = 0 )
                u, _, v = np_linalg.svd( ( data - mu ) / sigma )
                d3d = np_dot( data, v[:, :3] )
        else :
            d3d = np_zeros( ( data.shape[ 0 ], 3 ), np_float )
...
## Plot the data
        ...
        if len( self.__current_partition ) :
            ...
            for cl, pi in zip( [ 'r', 'g', 'b', 'm', 'k', 'c', 'y' ], self.__current_partition ) :
                ...
                ax.scatter( cur[np_logical_not( c_m ),0], cur[np_logical_not( c_m ),1], cur[np_logical_not( c_m ),2], marker = 'o', c = cl )
                ax.scatter( cur[c_m,0], cur[c_m,1], cur[c_m,2], marker = '^', c = cl )
        else :
## Highlight the datapoint which were reconsturcted
            kept_data = d3d[np_logical_not( missing_mask ) ]
...
## Commit to the device
    def get_cluster_info( self, num ) :
        ...
        return len( cur_inx ), len( c_mask[ c_mask ] ), np_mean( cur, axis = 0 )
...
## Plot other datapoints
        ...
        ax.scatter( oth[np_logical_not(o_mask),0], oth[np_logical_not(o_mask),1], oth[np_logical_not(o_mask),2], marker = 'o', c = '#D0D0D0' )
## Plot the contents of this cluster
        ax.scatter( cur[c_mask,0], cur[c_mask,1], cur[c_mask,2], marker = '^', c = 'red' )
        ax.scatter( cur[np_logical_not(c_mask),0], cur[np_logical_not(c_mask),1], cur[np_logical_not(c_mask),2], marker = 'o', c = 'red' )
## Mark the centroid (reconsturcted data)
        ccur = np_mean( cur, axis = 0 )
