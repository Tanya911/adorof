...
class initial_partition( object ) :
	def __init__( self ) :
		super(initial_partition, self).__init__( )
	def __call__( self, data, K = 5, **kwargs ) :
		return [ list( ) for c in range( K ) ]

## Private dependencies
from scipy.cluster.hierarchy import linkage
class initial_partition_hier( initial_partition ):
	"""Класс (initial_partition_hier) для порождения начального разбиения
	агломеративной иерархической кластеризацией. Использование:
		pi0 = initial_partition_hier()
		init_partition = pi0(
			data = данные,
	...
	def __init__( self ):
		super(initial_partition_hier, self).__init__( )
